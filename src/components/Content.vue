<template>
  <div class="container-fluid pad-sides">
      ...
      <Skills v-observe-visibility="{ callback: isSkillsViewable, once: true, intersection: { threshold: 0.7 } }" :class="{ 'visible animated fadeInRight': animateSkills, 'invisible': !animateSkills }"/>
      <ProjectList v-observe-visibility="{ callback: isProjectsViewable, once: true, intersection: { threshold: 0.3  } }" :class="{ 'visible animated slideInLeft': animateProjects, 'invisible': !animateProjects }" />
      <InterestsList v-observe-visibility="{ callback: isInterestsViewable, once: true, intersection: { threshold: 0.4  } }" :class="{ 'visible animated slideInRight': animateInterests, 'invisible': !animateInterests }" />
      <Contact v-observe-visibility="{ callback: isContactViewable, once: true, intersection: { threshold: 0.3  } }" :class="{ 'visible animated fadeInUp': animateContact, 'invisible': !animateContact }" />
  </div>
</template>
